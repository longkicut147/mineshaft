...
from mineshaft.game import Game
...
pygame.display.set_caption("Board game Coup")
...
player_A = Player("long")
player_B = Player("nam")
player_C = Player("khanh")
player_D = Player("hieu")
...
# Example chat log (could be dynamically updated in the game)
chat_log = [
    "Welcome to the game!", "Player A attacked Player B", "Player C used a wild card", "Player D gained 5 gold"
    # More chat updates could be added here
]
...
def main():
    global scroll_y
    running = True
    game.play()  # Start the game logic
    while running:
        ...
        # Update player data and draw board
        player_data = update_player_data()
        draw_board(player_data)
